...
enable_and_delete_topup() ->
    ...
    Resp = topup_request(AccountId, API, RequestEnvelope),
    ?INFO("enable topup resp: ~s", [Resp]),

    RespJObj = kz_json:get_value(<<"data">>, kz_json:decode(Resp)),
    ...
    Resp1 = topup_request(AccountId, API, RequestEnvelope1),
...
topup_request(AccountId, API, RequestEnvelope) ->
